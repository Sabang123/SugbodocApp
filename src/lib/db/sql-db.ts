...
class SqlDatabase {
  ...
  public auth = {
    ...
    signUp: async ({ email, password, options }: { email: string; password: string; options?: any }) => {
      ...
      const existingUser = this.getTable("profiles").find((p) => p.email?.toLowerCase() === email.toLowerCase());

      const userId = existingUser ? existingUser.id : `user-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 6)}`;
      ...
      const user = { id: userId, email, name, role: "patient" as AppRole };
      this.setCurrentSession(user);

      return {
        data: {
          user: { id: userId, email, user_metadata: { name } },
          session: { access_token: `token_${userId}`, user: { id: userId, email } },
        },
        error: null,
      };
    },
    signInWithPassword: async ({ email, password }: { email: string; password: string }) => {
      ...
      // If not in profiles, search in doctors
      ...
      if (!profile) {
        if (lower.includes("admin")) {
          ...
          // Default to Juan dela Cruz or auto-create profile
          profile = this.getTable("profiles")[0] || {
            id: "patient-juan-cruz",
            email: lower,
            name: "Juan dela Cruz",
            phone: "+63 917 123 4567",
